								...
								<div class="col-md-12">
									<div class="form-group m-t-30 m-b-5">
										<div class="col-xs-6 text-left">
											<a href="<?php echo e(action($sRow->sPath.'\PrepareController@show', $sRow->id)); ?>" class="btn btn-sm btn-success m-l-10">กลับไปหน้าใบเตรียมความพร้อม</a>
										</div>
									</div>
								</div>
							</form>
...
$(function() {
	...
	oTable = $('#data-table').DataTable({
        ...
        ajax: {
			url: '<?php echo e(action("SWP\PrepareForemanController@postDatatable", $sRow->id)); ?>',
			data: function ( d ) {  
				d.Where={};
	...
	oTableForeman = $('#data-table-foreman').DataTable({
		...
		scrollY: ''+($(window).height()-250)+'px',
        ajax: {
			url: '<?php echo e(action("SWP\PrepareForemanController@postDatatableWithWeight", $sRow->id)); ?>',
			data: function ( d ) {  
				d.Where={};
	...
	$( '#modal-foreman' ).on( "click", '.ForemanInsert', function() {
		if( $("#modal-foreman input[type='radio']:checked").val() === undefined ){
			...
			$.ajax({
				...
				url: '<?php echo e(action("SWP\PrepareForemanController@AjaxInsert", $sRow->id)); ?>',
	...
	$( '#modal-foreman-edit' ).on( "click", '.ForemanUpdate', function() {
		if(confirm('ยืนยันการทำรายการ ?') == true){
			$.ajax({
				...
				url: '<?php echo e(action("SWP\PrepareForemanController@AjaxUpdate", $sRow->id)); ?>',
				success: function(result) {
					if( result ){
						$('#modal-alert').modal('show');
						$('#modal-alert #msg').html(result.msg);
	...
	$( '#modal-foreman-edit' ).on( "click", '.ForemanDelete', function() {
		if(confirm('ยืนยันการทำรายการ\nยกเลิกโฟร์แมนในรายการปฎิบัติงานนี้ ?') == true){
			$.ajax({
				...
				url: '<?php echo e(action("SWP\PrepareForemanController@AjaxDelete", $sRow->id)); ?>',
				success: function(result) {
					if( result ){
						$('#modal-alert').modal('show');
						$('#modal-alert #msg').html(result.msg);
