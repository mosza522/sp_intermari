...
class OperationController extends Controller
{
    ...
    {
		return View('SWP.Operation.Index')->with(array('PageContainer'=> true) );
    }
    ...
    public function OperationShowStevieDore( $rsCLM ) 
    {
		$rsOperation 	= \App\Models\CLM\Operation::where('ItemID',$rsCLM->id)->where('workStatus','1')->orderBy('id', 'desc')->first();
	...
	public function postDatatable(Request $request){
		$sTable = \App\Models\SMD\SmdItem::join('ck_Smd', function ($join) {
					...
					->select('ck_Smd_Item.id', 'SmdID', 'job_number', 'work_number', 'job_date_eta', 'job_weight', 'ck_Smd_Item.created_at', 'job_transport',  'work_status_swp', 'BoatName', 'ck_Smd.deleted_at AS deleted_at1', 'ck_Smd_Item.deleted_at as deleted_at2')
					->where('work_prepare_swp', 'Y')
					->whereIn('work_mode', ['TRU', 'CLM', 'SWP'])
					->where('work_contractor', 'SWP');
		...
		->editColumn('work_status_swp',function($data){
			if($data->work_status_swp == 0 ) return 'รอดำเนินการ';
			if($data->work_status_swp == 1) return 'กำลังดำเนินการ';
			if($data->work_status_swp == 2 ) return 'จบงาน';
		});
