...
$(function() {
	// console.log(machine_id);
        ...
        ajax: {
			url: '{{ action("SWP\MachineController@checkInspectionDatatable") }}',
			data: function ( d ) {
				d.Where={};
      ...
      //
			...
			// for (var i = 0; i < machine_id.length; i++) {
				// if(aData['id']==machine_id[i]){
					...
					+ '<a href="{{action("SWP\MachineController@checkInspection")}}/'+aData['id']+'/'+(aData['deleted_at']?'?withTrashed=true':'')+'" style="margin:0px;">'+(aData['InspectionCode']?aData['InspectionCode']:'Wait')+'</a> '
					);

					// $('td:nth-child(2)', nRow).html(''
					// + '<a href="{{action("SWP\MachineController@checkInspection")}}/'+aData['id']+'/'+aData['MachineNunber']+'/check'+(aData['deleted_at']?'?withTrashed=true':'')+'" class="btn btn-xs btn-success" style="margin:0px;"><i class="glyphicon glyphicon-edit"></i> อนุมัติ</a> '
					// + '<a href="{{action("SWP\MachineController@checkInspection")}}/'+aData['id']+'/'+aData['MachineNunber']+'/check'+(aData['deleted_at']?'?withTrashed=true':'')+'" class="btn btn-xs btn-danger" style="margin:0px;"><i class="glyphicon glyphicon-edit"></i> ไม่อนุมัติ</a> '
          //
					// );
				// }
			// }
					...
					// + '<a href="{{action("SWP\MachineController@checkInspection")}}/'+aData['id']+'/'+aData['MachineNunber']+'/check'+(aData['deleted_at']?'?withTrashed=true':'')+'" class="btn btn-xs btn-primary" style="margin:0px;"><i class="glyphicon glyphicon-edit"></i> รอการตรวจเช็ค</a> '
					// );
				// }
        //
				// }
