...
$(function() {
	...
	oTable = $('#data-table').DataTable({
        ...
        ajax: {
			url: '{{ action("SWP\PrepareSweepController@postDatatable", $sRow->id) }}',
			data: function ( d ) {  
				d.Where={};
	...
	oTableSweep = $('#data-table-sweep').DataTable({
		...
		scrollY: ''+($(window).height()-250)+'px',
        ajax: {
			url: '{{ action("SWP\PrepareSweepController@postDatatableWithWeight", $sRow->id) }}',
			data: function ( d ) {  
				d.Where={};
	...
	$( '#modal-sweep' ).on( "click", '.SweepInsert', function() {
		if( $("#modal-sweep input[type='radio']:checked").val() === undefined ){
			...
			$.ajax({
				type: 'POST',
				dataType: 'JSON',
				data: { 'SweepID' : $("#modal-sweep input[type='radio']:checked").val() },
				url: '{{ action("SWP\PrepareSweepController@AjaxInsert", $sRow->id) }}',
	...
	$( '#modal-sweep-edit' ).on( "click", '.SweepUpdate', function() {
		if(confirm('ยืนยันการทำรายการ ?') == true){
			$.ajax({
				...
				url: '{{ action("SWP\PrepareSweepController@AjaxUpdate", $sRow->id) }}',
				success: function(result) {
					if( result ){
						$('#modal-alert').modal('show');
						$('#modal-alert #msg').html(result.msg);
	...
	$( '#modal-sweep-edit' ).on( "click", '.SweepDelete', function() {
		if(confirm('ยืนยันการทำรายการ\nยกเลิกสายกวาดในรายการปฎิบัติงานนี้ ?') == true){
			$.ajax({
				...
				url: '{{ action("SWP\PrepareSweepController@AjaxDelete", $sRow->id) }}',
				success: function(result) {
					if( result ){
						$('#modal-alert').modal('show');
						$('#modal-alert #msg').html(result.msg);
