							...
							<fieldset class="m-t-10">
								...
								<div class="col-md-12 m-b-20" style="min-height:200px;">
									<div class="p-l-0 text-left pull-right">
										<a href="#modal-EquipmentNew" class="btn btn-sm btn-primary" data-toggle="modal" id="EquipmentNew">เพิ่มเครื่องจักร และ อุปกรณ์ไปในรายการปฎิบัติงาน</a>
									</div>
...
$(function() {
	...
	oTable = $('#data-table').DataTable({
        ...
        ajax: {
			url: '{{ action("SWP\PrepareMachineController@postDatatable", $sRow->id) }}',
			data: function ( d ) {  
				d.Where={};
	...
	oTableEquipment = $('#data-table-equipment').DataTable({
		...
		scrollY: ''+($(window).height()-250)+'px',
        ajax: {
			url: '{{ action("SWP\PrepareMachineController@postDatatableWithWeight", $sRow->id) }}',
			data: function ( d ) {  
				d.Where={};
	...
	$( '#modal-EquipmentNew' ).on( "click", '.EquipmentSave', function() {
		if( $("#modal-EquipmentNew input[type='radio']:checked").val() === undefined ){
			alert('กรุณาเลือกเครื่องจักรในรายการปฎิบัติงาน');
		}else{
			
			$.ajax({
				type: 'POST',
				dataType: 'JSON',
				data: { 'MachineID' : $("#modal-EquipmentNew input[type='radio']:checked").val() },
				url: '{{ action("SWP\PrepareMachineController@AjaxInsert", $sRow->id) }}',
				success: function(result) {
					if( result ){
						...
						if( result.status == 'Success' ){
							oTable.draw(false);
							oTableEquipment.draw(false);
						}
						$('#modal-EquipmentNew').modal('hide');
					}
				}
			});
			$("#modal-EquipmentNew input[type='radio']:checked").prop('checked', false)
		}
	});
	
	$( '#data-table' ).on( "click", '.EquipmentEdit', function() {
		if( $(this).parent().data('aData').MachineName ){
	...
	$( '#modal-EquipmentEdit' ).on( "click", '.EquipmentSave', function() {
		if(confirm('ยืนยันการทำรายการ ?') == true){
			$.ajax({
				...
				url: '{{ action("SWP\PrepareMachineController@AjaxUpdate", $sRow->id) }}',
				success: function(result) {
					if( result ){
						$('#modal-alert').modal('show');
						$('#modal-alert #msg').html(result.msg);
	...
	$( '#modal-EquipmentEdit' ).on( "click", '.EquipmentDelete', function() {
		if(confirm('ยืนยันการทำรายการ\nยกเลิกเครื่องจักรและอุปกรณ์นี้?') == true){
			$.ajax({
				...
				url: '{{ action("SWP\PrepareMachineController@AjaxDelete", $sRow->id) }}',
				success: function(result) {
					if( result ){
						$('#modal-alert').modal('show');
						$('#modal-alert #msg').html(result.msg);
