                        ...
                        <div class="panel-body">
							<div class="row" style="padding: 0px 2px 0 0;">
								<div class="col-md-6 text-left" >
									<input class="form-control input-sm w250 text-center myLike" name="BuoyName" type="text" placeholder="ข้อมูลทุ่น">
								</div>
								<div class="col-md-6 text-right" >
									<a href="{{ action('Master\BuoyController@create') }}" class="btn btn-sm btn-success">เพิ่มข้อมูลทุ่น</a>
								</div>
							</div>
...
$(function() {
        ...
        ajax: {
			url: '{{ action("Master\BuoyController@postDatatable") }}',
			data: function ( d ) {  
				d.Where={};
        ...
        columns: [
            ...
            {data: 'BuoyName', title :'ข้อมูลทุ่น', className: 'text-center'},
		...
		rowCallback: function(nRow, aData, dataIndex){
			...
			$('td:last-child', nRow).html(''
				+ '<a href="{{ action("Master\BuoyController@index") }}/'+aData['id']+'/edit" class="btn btn-xs btn-primary" style="margin:0px;"><i class="glyphicon glyphicon-edit"></i> แก้ไข</a> '
				+ '<a rel="{{ action("Master\BuoyController@index") }}/'+aData['id']+'/" class="btn btn-xs btn-danger cDelete" style="margin:0px;"><i class="glyphicon glyphicon-remove"></i> ลบ</a> '
			);
			@if (session('RecordID'))
