                        ...
                        <div class="panel-body">
							<div class="row" style="padding: 0px 2px 0 0;">
								<div class="col-md-9 text-left" >
									...
									<div class="pull-left m-l-10"><input class="form-control input-sm w250 text-center myLike" name="MachineType" type="text" placeholder="ประเภท"></div>
								...
								<div class="col-md-3 text-right" >
									<a href="{{ action('Master\FTS\MachineController@create') }}" class="btn btn-sm btn-success">เพิ่มเครื่องจักร</a>
								</div>
							</div>
...
$(function() {
		...
		rowCallback: function(nRow, aData, dataIndex){
			if( aData['deleted_at'] ){
				...
				$('td:last-child', nRow).html('<a rel="{{ action("Master\FTS\MachineController@index") }}/'+aData['id']+'/?withTrashed=true" class="btn btn-xs btn-warning cDelete" style="margin:0px;"><i class="fa fa-trash-o"></i> กู้คืนข้อมูลที่ลบ</a>');
			}else{
				$('td:last-child', nRow).html(''
					+ '<a href="{{ action("Master\FTS\MachineController@index") }}/'+aData['id']+'/edit" class="btn btn-xs btn-primary" style="margin:0px;"><i class="glyphicon glyphicon-edit"></i> แก้ไข</a> '
					+ '<a rel="{{ action("Master\FTS\MachineController@index") }}/'+aData['id']+'/" class="btn btn-xs btn-danger cDelete" style="margin:0px;"><i class="glyphicon glyphicon-remove"></i> ลบ</a> '
				);
			}
			@if (session('RecordID'))
