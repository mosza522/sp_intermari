                        ...
                        <div class="panel-body">
							<div class="row" style="padding: 0px 2px 0 0;">
								...
								<div class="col-md-2 text-right" >
									<a href="{{ action('SMD\SMDController@create') }}" class="btn btn-sm btn-success">เพิ่มใบสั่งขาย (SALE ORDER)</a>
								</div>
...
$(function() {
		...
		iDisplayLength: 10,
        ajax: {
			url: '{{ action("SMD\SMDController@postDatatable") }}',
			data: function ( d ) {
				d.Where={};
		...
		rowCallback: function(nRow, aData, dataIndex){
			...
			$('td:last-child', nRow).html(''
				+ '<a href="{{ action("SMD\SMDController@index") }}/'+aData['id']+'/edit'+(aData['deleted_at']?'?withTrashed=true':'')+'" class="btn btn-xs btn-primary" style="margin:0px;"><i class="glyphicon glyphicon-edit"></i> แก้ไข</a> '
				+ '<a rel="{{ action("SMD\SMDController@index") }}/'+aData['id']+'/" class="btn btn-xs btn-danger cDelete" style="margin:0px;"><i class="glyphicon glyphicon-remove"></i> ลบ</a> '
			);
