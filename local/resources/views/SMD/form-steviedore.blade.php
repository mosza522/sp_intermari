...
<div id="content" class="content">
	...
	<ol class="breadcrumb pull-right">
		...
		<li>ใบสั่งขาย</li>
		<li class="active">@if(empty($sRowItem)) เพิ่มใบแจ้งงานสตีวีโดร์ @else ใบแจ้งงานสตีวีโดร์ : {{ $sRowItem->work_number }} @endif</li>
	</ol>
	<!-- end breadcrumb -->
	<!-- begin page-header -->
	<h1 class="page-header">ใบแจ้งงานสตีวีโดร์</h1>
	<!-- end page-header -->
	...
	<div class="row">
		<!-- begin col-12 -->
		<div class="col-md-12">
			<form action="{{action('SMD\SMDController@saveStevieDore', $sRow->id )}}" method="POST" autocomplete="off">
			{{ csrf_field() }}
				...
				<div class="panel @if(empty($sRowItem)) panel-success @elseif( !empty($sRow->deleted_at) || !empty($sRowItem->deleted_at) ) panel-danger @else panel-inverse @endif ">
					<div class="panel-heading">
						...
						<div class="panel-heading-btn">
							<a href="{{ action('SMD\SMDController@StevieDore',$sRow->id) }}" class="btn btn-sm  btn-icon btn-circle btn-success"><i class="fa fa-plus"></i></a>
							<a href="javascript:;" class="btn btn-sm  btn-icon btn-circle btn-danger cConfirm" rel="{{ action('SMD\SMDController@ItemRemove', array($sRow->id, $sRowItem->id, 'StevieDore')) }}" msg="ยืนยันการลบใบแจ้งงานสตีวีโดร์ {{ $sRowItem->work_number }}?" ><i class="fa fa-times"></i></a>
						</div>
						@endif
						<h4 class="panel-title"> @if(empty($sRowItem)) เพิ่มใบแจ้งงานสตีวีโดร์ @else ใบแจ้งงานสตีวีโดร์ : {{ $sRowItem->work_number }} @endif</h4>
					</div>
					<div class="panel-body form-horizontal">
						<div class="row">
							<div class="col-md-6">
								...
								<div class="form-group m-b-10">
									<label class="col-md-3 control-label">เลขที่ใบแจ้งงานสตีวีโดร์</label>
									<div class="row col-md-9">
										<div class="pull-left m-l-10" style="width: 32%;">
								...
								<div class="form-group m-b-10">
									...
									<div class="col-md-8">
										<select class="form-control w100" name="work_machine">
											<option value="0" {{ @$sRowItem->work_machine=='0'?'selected':'' }}>No</option>
											<option value="1" {{ (@$sRowItem->work_machine=='1' || empty($sRowItem))?'selected':'' }}>Yes</option>
										</select>
									</div>
								</div>
								...
								<div class="form-group m-b-10">
									...
									<div class="col-md-9">
										<div class="pull-left w100">
											<input type="text" name="work_crane_number" class="form-control num text-right " placeholder="จำนวนเครน" value="{{ @$sRowItem->work_crane_number }}"/>
										</div>
										<div class="pull-left w40">
											<input type="text" class="form-control" value="ตัว" readonly style="border: transparent;background-color: white;color: black;opacity: 1;" />
										</div>
										<div class="pull-left w70">
											<select class="form-control" name="work_crane_weight">
												<option value="15" {{ @$sRowItem->work_crane_weight=='15'?'selected':'' }}>15</option>
												<option value="20" {{ @$sRowItem->work_crane_weight=='20'?'selected':'' }}>20</option>
												<option value="25" {{ @$sRowItem->work_crane_weight=='25'?'selected':'' }}>25</option>
												<option value="30" {{ (@$sRowItem->work_crane_weight=='30' || empty($sRowItem))?'selected':'' }}>30</option>
												<option value="35" {{ @$sRowItem->work_crane_weight=='35'?'selected':'' }}>35</option>
											</select>
										</div>
										<div class="pull-left w100">
											<input type="text" class="form-control" value="น้ำหนักยก (ตัน)" readonly style="border: transparent;background-color: white;color: black;opacity: 1;" />
										</div>
									</div>
								</div>
							</div>
							...
							<div class="col-md-6">
								...
								<div class="form-group m-b-10">
									...
									<div class="col-md-8">
										<select class="form-control w100" name="work_grab">
											<option value="0" {{ @$sRowItem->work_grab=='0'?'selected':'' }}>No</option>
											<option value="1" {{ (@$sRowItem->work_grab=='1' || empty($sRowItem))?'selected':'' }}>Yes</option>
										</select>
									</div>
								</div>
								...
								<div class="form-group m-b-10">
									...
									<div class="col-md-8">
										<select class="form-control w100" name="work_sealine">
											...
											<option value="1" {{ (@$sRowItem->work_sealine=='1' || empty($sRowItem))?'selected':'' }}>Yes</option>
										</select>
									</div>
								</div>
							...
							<div class="col-md-12">
								<div class="form-group m-t-30 m-b-5">
									...
									<div class="col-xs-4 text-center">
										...
										<button type="submit" class="btn btn-sm btn-primary m-r-5">@if(empty($sRowItem)) เพิ่มใบแจ้งงานสตีวีโดร์ @else ปรับปรุงใบแจ้งงานสตีวีโดร์ @endif </button>
										@endif
									</div>
