...
@section('content')
		...
		<div id="content" class="content">
			...
			<!-- end breadcrumb -->
			<!-- begin page-header -->
			<h1 class="page-header">ใบเตรียมความพร้อมโฟร์แมน</h1>
			<!-- end page-header -->
...
$(function() {
	...
	oTable = $('#data-table').DataTable({
        ...
        ajax: {
			url: '{{ action("FTS\PrepareForemanController@postDatatable", $sRow->id) }}',
			data: function ( d ) {  
				d.Where={};
	...
	oTableForeman = $('#data-table-foreman').DataTable({
		...
		scrollY: ''+($(window).height()-250)+'px',
        ajax: {
			url: '{{ action("FTS\PrepareForemanController@postDatatableWithWeight", $sRow->SmdID) }}',
			data: function ( d ) {  
				d.Where={};
	...
	$( '#modal-foreman' ).on( "click", '.ForemanInsert', function() {
		if( $("#modal-foreman input[type='radio']:checked").val() === undefined ){
			...
			$.ajax({
				type: 'POST',
				dataType: 'JSON',
				data: { 'OperationID' : {{ $sRow->OperationID }}, 'ForemanID' : $("#modal-foreman input[type='radio']:checked").val() },
				url: '{{ action("FTS\PrepareForemanController@AjaxInsert", $sRow->id) }}',
	...
	$( '#modal-foreman-edit' ).on( "click", '.ForemanUpdate', function() {
		if(confirm('ยืนยันการทำรายการ ?') == true){
			$.ajax({
				...
				url: '{{ action("FTS\PrepareForemanController@AjaxUpdate", $sRow->id) }}',
				success: function(result) {
					if( result ){
						$('#modal-alert').modal('show');
						$('#modal-alert #msg').html(result.msg);
	...
	$( '#modal-foreman-edit' ).on( "click", '.ForemanDelete', function() {
		if(confirm('ยืนยันการทำรายการ\nยกเลิกโฟร์แมนในรายการปฎิบัติงานนี้ ?') == true){
			$.ajax({
				...
				url: '{{ action("FTS\PrepareForemanController@AjaxDelete", $sRow->id) }}',
				success: function(result) {
					if( result ){
						$('#modal-alert').modal('show');
						$('#modal-alert #msg').html(result.msg);
