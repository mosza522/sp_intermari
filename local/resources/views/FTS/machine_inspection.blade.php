...
$(function() {
	// console.log(machine_id);
        ...
        ajax: {
			url: '{{ action("FTS\MachineController@postInspection") }}',
			data: function ( d ) {
				d.Where={};
      ...
      //
			...
			// for (var i = 0; i < machine_id.length; i++) {
				// if(aData['id']==machine_id[i]){
					...
					+ '<a href="{{action("FTS\MachineController@inspection")}}/'+aData['id']+'/check'+(aData['deleted_at']?'?withTrashed=true':'')+'" style="margin:0px;">'+(aData['InspectionCode']?aData['InspectionCode']:aData['id'])+'</a> '
					);

					// $('td:nth-child(2)', nRow).html(''
					// + '<a href="{{action("FTS\MachineController@inspection")}}/'+aData['id']+'/'+aData['MachineNunber']+'/check'+(aData['deleted_at']?'?withTrashed=true':'')+'" class="btn btn-xs btn-success" style="margin:0px;"><i class="glyphicon glyphicon-edit"></i> อนุมัติ</a> '
					// + '<a href="{{action("FTS\MachineController@inspection")}}/'+aData['id']+'/'+aData['MachineNunber']+'/check'+(aData['deleted_at']?'?withTrashed=true':'')+'" class="btn btn-xs btn-danger" style="margin:0px;"><i class="glyphicon glyphicon-edit"></i> ไม่อนุมัติ</a> '
          //
					// );
				// }
			// }
					...
					// + '<a href="{{action("FTS\MachineController@inspection")}}/'+aData['id']+'/'+aData['MachineNunber']+'/check'+(aData['deleted_at']?'?withTrashed=true':'')+'" class="btn btn-xs btn-primary" style="margin:0px;"><i class="glyphicon glyphicon-edit"></i> รอการตรวจเช็ค</a> '
					// );
				// }
        //
				// }
