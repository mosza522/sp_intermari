...
@section('data-table')
	<div class="col-md-12 m-t-25">
		<div class="p-l-0 text-left pull-right">
			<a href="{{ action('FTS\OperationOilBuoyController@create', $sRow->id) }}" class="btn btn-sm btn-primary">บันทึกการใช้น้ำมันของทุ่น</a>
		</div>
...
$(function() {
		...
		iDisplayLength: 10,
        ajax: {
			url: '{{ action("FTS\OperationOilBuoyController@postDatatable",$sRow->id) }}',
			data: function ( d ) { },
            method: 'POST'
        },
        columns: [
		...
		rowCallback: function(nRow, aData, dataIndex){
			...
			$('td:last-child', nRow).html(''
				+ '<a href="{{ action("FTS\OperationOilBuoyController@index",$sRow->id ) }}/'+aData['id']+'" class="btn btn-xs btn-primary" style="margin:0px;"><i class="fa fa-eye"></i> เปิดเอกสาร</a> '
			);
		}
    });
